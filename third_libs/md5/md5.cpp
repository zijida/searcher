...
namespace md5
{
	...
	exception.
	*****************************************************************************************/
	void cmd5::getmd5(std::string& context,unsigned char* arrmd5)
	{
		getmd5((BYTE*)context.c_str(),context.length(),arrmd5);
	}
	...
	*****************************************************************************************/
	//void cmd5::getmd5(CFile& File,unsigned char* arrmd5)
	//{
	//	// 		try
	//	// 		{
	...
	*****************************************************************************************/
	void cmd5::getmd5(BYTE* pBuf, UINT nLength,unsigned char* arrmd5)
	{
		//calculate and return the checksum
		cmd5 md5maker;
		md5maker.update( pBuf, nLength );
		md5maker.final(arrmd5);
	}


	/*****************************************************************************************
	FUNCTION:  cmd5::rotate
	DETAILS:  private
	DESCRIPTION: Rotates the bits in a 32 bit UINT4 left by a specified amount
	RETURNS:  The rotated UINT4 
	ARGUMENTS:  UINT4 x : the value to be rotated
	int n   : the number of bits to rotate by
	*****************************************************************************************/
	UINT4 cmd5::rotate(UINT4 x, int n)
	{
		//rotate and return x
		return (x << n) | (x >> (32-n));
	}
	...
	RETURNS:  none
	ARGUMENTS:  UINT4 &A, B, C, D : Current (partial) checksum
	UINT4 X           : Input data
	UINT4 S     : MD5_SXX Transformation constant
	UINT4 T     : MD5_TXX Transformation constant
	NOTES:   None
	*****************************************************************************************/
	void cmd5::FF( UINT4& A, UINT4 B, UINT4 C, UINT4 D, UINT4 X, UINT4 S, UINT4 T)
	{
		UINT4 F = (B & C) | (~B & D);
		A += F + X + T;
		A = rotate(A, S);
	...
	NOTES:   None
	*****************************************************************************************/
	void cmd5::GG( UINT4& A, UINT4 B, UINT4 C, UINT4 D, UINT4 X, UINT4 S, UINT4 T)
	{
		UINT4 G = (B & D) | (C & ~D);
		A += G + X + T;
		A = rotate(A, S);
	...
	NOTES:   None
	*****************************************************************************************/
	void cmd5::HH( UINT4& A, UINT4 B, UINT4 C, UINT4 D, UINT4 X, UINT4 S, UINT4 T)
	{
		UINT4 H = (B ^ C ^ D);
		A += H + X + T;
		A = rotate(A, S);
	...
	NOTES:   None
	*****************************************************************************************/
	void cmd5::II( UINT4& A, UINT4 B, UINT4 C, UINT4 D, UINT4 X, UINT4 S, UINT4 T)
	{
		UINT4 I = (C ^ (B | ~D));
		A += I + X + T;
		A = rotate(A, S);
	...
	*****************************************************************************************/
	void cmd5::byte2dword(UINT4* Output, BYTE* Input, UINT nLength)
	{
		//initialisations
	...
	*****************************************************************************************/
	...
	/*****************************************************************************************
	FUNCTION:  cmd5::dword2byte
	DETAILS:  private
	DESCRIPTION: Transfers the data in an 32 bit array to a 8 bit array
	RETURNS:  void
	ARGUMENTS:  BYTE* Output  : the 8 bit destination array 
	UINT4* Input  : the 32 bit source array
	UINT nLength  : the number of 8 bit data items in the source array
	NOTES:   One UINT4 from the input array is transferred into four BYTES 
	in the output array. The first (0-7) bits of the first UINT4 are 
	...
	*****************************************************************************************/
	void cmd5::dword2byte(BYTE* Output, UINT4* Input, UINT nLength )
	{
		//transfer the data by shifting and copying
		UINT i = 0;
		UINT j = 0;
	...
	*****************************************************************************************/
	void cmd5::final(unsigned char* arrmd5)
	{
		//Save number of bits
		BYTE Bits[8];
		...
		dword2byte( arrmd5,m_lMD5,nMD5Size );
	}
